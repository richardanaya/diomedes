...
// Session lifecycle
// ============================================================
function createSession({ plot, aesthetic }) {
  ...
  const session = {
    ...
    historyIndex: -1,
    sceneVersion: 0,        // bumped on every committed scene; gates prefetch
    prefetch: new Map(),    // elementId -> { version, payload, stateSnapshot }
    createdAt: Date.now(),
    updatedAt: Date.now(),
  };
