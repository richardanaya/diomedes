...
const OPENING_TOOL = {
  ...
  function: {
    ...
    parameters: {
      type: 'object',
      properties: {
        title: { type: 'string', description: 'Short evocative title — literary, specific to this story, not generic ("The Mystery" is weak; "The Clockmaker\'s Debt" is strong).' },
        goal: {
          type: 'string',
        ...
        objectives: {
          ...
          items: {
            ...
            properties: {
              ...
              text: { type: 'string', description: 'Player-facing objective — one clear, concrete sentence. Name the thing to find, place to reach, or person to confront.' },
            },
            required: ['id', 'text'],
          },
        },
        ...
        opening_narrative: {
          type: 'string',
          description:
            'Opening scene prose: 3–5 sentences, second-person present tense. Ground the player in this specific place with sensory detail, tension, and voice. This is the first thing they read — make it sing.',
        },
        render_brief: {
          type: 'string',
          description:
    ...
    parameters: {
      ...
      properties: {
        narrative: {
          type: 'string',
          description:
            'Beat prose shown to the player: 2–4 sentences, second-person present tense. Describe what just happened and the new moment they are in — concrete, vivid, no filler. Story fiction only, not a summary.',
        },
        outcome: { type: 'string', description: 'A concise factual summary of what actually happened (for memory — plain prose, not shown to player).' },
        state_changes: {
          type: 'object',
          properties: {
        ...
        ending: {
          ...
          properties: {
            ...
            text: {
              type: 'string',
              description:
                'Closing narration: 2–4 sentences with emotional weight. Second-person present tense. Land the ending — triumph, tragedy, or bitter ambiguity — with a final image, not a recap.',
            },
          },
          required: ['is_ending'],
        },
...
6b. Make every beat CONSEQUENTIAL. A player action should produce a real change: reveal a clue or fact, hand over or take an item, shift a relationship, open a path, raise the stakes, or trigger a turn in the plot. Avoid empty filler beats where nothing of substance happens — always move the story forward.
6c. Tie every beat to the objective arc. Each beat should advance the CURRENT active objective (or the main goal if sub-objectives are done). Mark objectives_done in arc_update when an objective is genuinely completed — do not leave objectives open when the player has already accomplished them.
...
8. RARE INSPECT ACTIONS: only when is_inspect_action is true (uncommon — fine-detail clue the player must read up close). The render_brief must describe an EXTREME CLOSE-UP of that specific marking or inscription filling the frame. For all other actions, render a normal wide scene change even if the player is examining something.`;
...
Establish the goal, 2-4 objectives, the protagonist's locked look, the opening location, an opening narrative written to the WRITING STANDARD above, a precise render brief for the first image, and the initial story bible.`,
    tool: OPENING_TOOL,
...
function formatObjectiveFocus(summary) {
  const focus = summary?.objectiveFocus;
  if (!focus) return '';
  const lines = [`Goal: ${focus.goal}`];
  if (focus.currentObjective) {
    lines.push(`Current objective: [${focus.currentObjective.id}] ${focus.currentObjective.text}`);
  }
  if (focus.activeObjectives?.length) {
    lines.push('Active: ' + focus.activeObjectives.map(o => `[${o.id}] ${o.text}`).join('; '));
  }
  if (focus.completedObjectives?.length) {
    lines.push('Done: ' + focus.completedObjectives.map(o => `[${o.id}] ${o.text}`).join('; '));
  }
  return lines.join('\n');
}
...
async function directBeat({ summary, element }) {
  const args = await callTool({
    system: SYSTEM,
    user: `Objective focus (this beat must advance one of these):
${formatObjectiveFocus(summary)}
...
- Inspect action (close-up shot): ${element.is_inspect_action ? 'YES — next image must be an extreme close-up of this subject filling the frame' : 'no'}
...
Resolve this action. Apply real consequences, keep everything consistent with the story bible, advance the arc, and write beat narrative prose to the WRITING STANDARD above. Write a precise render brief for the next scene. End the story if the goal is now achieved or lost — ending text must also meet the WRITING STANDARD.`,
    tool: BEAT_TOOL,
